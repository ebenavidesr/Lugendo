...
- After pushing, also deploy to production automatically (same standing authorization, no confirmation needed): frontend via `pnpm --filter @workspace/lugendo-app run deploy` (`wrangler deploy` to Cloudflare Workers). Note: `artifacts/lugendo-app/wrangler.jsonc` documents that Cloudflare Workers Builds (dashboard-linked CI) may already auto-build+deploy from a `main` push independently — check there before assuming a manual deploy is needed. The backend (Railway) likely auto-deploys from `main` via its own GitHub integration. If `wrangler` isn't authenticated locally and no `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` is available, deploying isn't possible from this environment — say so rather than silently skipping it.
...
- Transactional emails (Resend, Notion #145): sending from `lugendo.io` requires the domain to be verified in the Resend dashboard (SPF/DKIM records added in Cloudflare DNS, where the domain's DNS is already managed) — this is a manual step in Resend/Cloudflare, not automatable from code. Until it's done, sends to real addresses fall back to Resend's sandbox sender and non-testing recipients are rejected with a 422 (visible in the `email_send_log` table's `error_message`).
