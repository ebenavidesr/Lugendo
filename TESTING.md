...
- [ ] Limpieza de las 3 filas ya guardadas con el menú de navegación (`country_advisories` donde `country_name` es `SN`, `ES` o `LK`) — identificadas pero no borradas: el classifier de seguridad del entorno bloqueó el `DELETE` directo contra la base real; pendiente de que tú lo confirmes o lo hagas manualmente. Son filas huérfanas (ya no se van a volver a consultar tras este fix) e inofensivas mientras tanto, no bloquean la corrección
- [ ] Verificación visual en navegador de que "Viaja Seguro" ya no muestra el bloque de navegación — pendiente por falta de credenciales de viajero funcionales en local; recomendado revisar en producción los viajes afectados (trip_id 2 Sri Lanka, 44 Ruanda/Uganda, 46 Senegal, que tienen días con código de país)
