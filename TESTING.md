...
- [ ] **Recorrido de los 4 pasos nuevos no verificado visualmente todavía** (el recorrido de arriba fue sobre la versión de 7 pasos) — pendiente: Origen → Programa → "Datos del viaje" (nombre+fechas+capacidad+descripción juntos) → "Crear" (itinerario detallado+invitaciones+resumen), y confirmar que añadir un vuelo después de crear el viaje desde la ficha (`FlightEditPanel`) sigue funcionando
