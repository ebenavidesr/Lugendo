...
- [ ] **Import PDF/DOCX/XLSX no probado** — el navegador solo permite subir archivos ya compartidos con la sesión, y no había ninguno disponible; el panel de subida (`ItineraryUploadPanel`) se verificó visualmente (dropzone, textos) pero no el flujo completo de análisis con IA ni la pantalla de asignación día-a-día de hotel/actividad (con los campos de teléfono/web/categoría ya unificados)
- [ ] Agencia (admin/manager/agent): no probado — sin cuenta de agencia disponible en esta sesión. Falta: crear viaje desde itinerario de catálogo existente, desde cero, por importación PDF, paso Vuelos, paso Invitaciones
...
- [ ] Toggle de noche en tránsito — no probado (requiere días de itinerario, que en el wizard solo aparecen tras importar un PDF)
