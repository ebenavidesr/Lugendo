...
- [ ] Filtro de "solo agencias activas" en el selector: Quique reporta que "Lugendo Demo" sigue apareciendo y no debería — verificado que el filtro (`allAgencies.filter(a => a.active)`) funciona correctamente a nivel de código (ya lo mostraba antes de esta ronda, con una agencia de prueba desactivada), y que en la base de datos "Lugendo Demo" tiene `active: true` ahora mismo. La discrepancia es de expectativa de dato, no de código: en la ronda 7 se "restauró" `active: true` para esta agencia al corregir una corrupción de datos, asumiendo que el valor `false` encontrado entonces formaba parte de esa corrupción — sin confirmar con Quique si ese `false` era en realidad el estado correcto/intencional. Pendiente de que Quique confirme si Lugendo Demo debe quedar activa o inactiva antes de tocar el dato de nuevo
- [ ] Viñetas ("bullet points") en el editor de texto enriquecido: Quique reporta no poder insertarlas. No reproducido en el navegador de pruebas (Chromium vía CDP) — el comando del botón (`document.execCommand('insertUnorderedList')`) funciona correctamente ahí, tanto sobre texto ya escrito como en el campo vacío. Pendiente de que Quique confirme navegador y pasos exactos para reproducirlo — el editor usa `execCommand`, una API obsoleta con comportamiento inconsistente entre navegadores (especialmente Safari), así que es plausible que sea un problema específico de su navegador
