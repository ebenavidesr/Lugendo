...
> Auditoría: `/buscar` (#161) y el perfil público de agencia (#162) solo muestran tarjetas resumen de itinerario (foto/nombre/precio), no la vista día a día — no había conflicto real con el trabajo en curso de esas dos tarjetas pese al aviso inicial. El único sitio con el patrón antiguo era `itinerary-detail.tsx` + el modo `itinerary` de `day-activities-panel.tsx`; `trip-day-card.tsx` (viajero) y el modo `trip` de `day-activities-panel.tsx` (back office) ya seguían el patrón de #159 y no se han tocado. Detalle completo de la auditoría y de la decisión sobre el coste en modo itinerario, en la tarjeta #171 de Notion.
- [ ] La ficha de itinerario (back office) usa el mismo patrón visual que la vista de viaje del viajero (#159): fila con foto cuadrada, actividades colapsadas con expansión individual en acordeón — implementado (`itinerary-detail.tsx`, `day-activities-panel.tsx`), pendiente de verificación visual en navegador por falta de credenciales de login funcionales en local (ver nota sobre `admin@lugendo.io` al final de este archivo)
- [ ] "Día N" en itinerario se muestra sin fecha absoluta; en viaje se sigue mostrando con fecha, sin regresión — implementado (`itinerary-detail.tsx` no añade fecha; `trip-detail.tsx`/`trip-day-card.tsx` no se tocaron), pendiente de verificación visual
- [ ] Comportamiento del coste en actividades "por libre" en modo itinerario es coherente con la decisión documentada en la tarjeta — decisión: se muestra igual que en viaje (coste de referencia/plantilla), ver justificación en la tarjeta de Notion
- [ ] La vista de viaje del viajero (#159) no sufre ninguna regresión visual tras el refactor de componente compartido — verificado por lectura de código que la rama `trip` de `day-activities-panel.tsx` y `trip-day-card.tsx` no cambiaron; sin verificación visual
...
- [x] `pnpm run typecheck` pasa sin errores nuevos — verificación visual en navegador pendiente (ver nota de credenciales)
...
- [x] Panel de gestión: toggle "Publicado en el buscador" (ahora enmarcado como "actívalo por defecto, desactívalo si no quieres aparecer"), selector de tipo de viaje y precio orientativo — **movidos** del diálogo de edición en `/itineraries` (donde no se encontraban con facilidad) a una tarjeta "Catálogo público" en la página de detalle del itinerario (`/itineraries/:id`), justo debajo de la cabecera — implementado y typecheck limpio, pendiente de verificación visual por clic (sin credenciales de login en local)
