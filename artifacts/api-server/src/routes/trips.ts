...
router.post("/trips", requireRoles("admin", "manager", "agent"), async (req, res): Promise<void> => {
  const { name, itineraryId, startDate, endDate, maxCapacity, airline, flightNumber, flightTime, reservationCode, flightNotes } = req.body;
  ...
  const [trip] = await db
    .insert(tripsTable)
    .values({ agencyId, itineraryId, name, startDate, endDate, maxCapacity, airline, flightNumber, flightTime, reservationCode, flightNotes, createdBy: req.session.userId })
    .returning();
